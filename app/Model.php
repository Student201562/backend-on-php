...
// изменяется элемент по задонному индексу
	public function save($id, $item){
        $data = file_get_contents($this->dataFileName); // читает содержимое файла в строку
	    $data = json_decode($data);
	    if(array_key_exists($id, $data)){
	        $data[$id] = $item;
        }
        return file_put_contents($this->dataFileName, json_encode($data)); // записывает строку в файл
	}
	...
	public function delete($id){
		$data = file_get_contents($this->dataFileName);
        $data=json_decode($data);
        unset($data[$id]); // удаляет переменную
        return file_put_contents($this->dataFileName, json_encode($data));
	}
}